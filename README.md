...
1. Head to the GiHUb Actions tab, and click on the `Build Revenge` workflow. Click on `Run workflow`, and fill in the required information. The `DISCORD_IPA` field should be the direct link to the decrypted Discord. The ipa will be built and uploaded to the artifacts tab.
